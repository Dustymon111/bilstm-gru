...
def predict():
    ...
    st.markdown(
        """
        <h1 style="text-align: center;">Evaluasi Prediksi Harga Saham</h1>
        """,
    ...
    ## Calendar ## 
    # start_index = 30
    # end_index = len(valid_dates) - 29   
    # valid_weekday_dates = [date for date in valid_dates[start_index:end_index] if date.weekday() < 5]
    # min_date = valid_weekday_dates[0]
    # max_date = valid_weekday_dates[-1]
    # start_predict_date = col2.date_input(
    #     "Tanggal Mulai Prediksi",
    #     min_value=min_date,  # earliest valid date
    #     max_value=max_date,  # latest valid date
    ...
    if st.button("Prediksi Harga",use_container_width=True):
        try:
            if df.empty:
                ...
                prediction_df["Actual"] = prediction_df["Actual"].astype(int)
